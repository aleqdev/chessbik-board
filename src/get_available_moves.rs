...
use crate::{shape_geodesic_field, Board, PieceColor, PieceMove, PiecePosition, PieceTy};

pub trait GetAvailableMoves {
    fn get_available_moves(&self, pos: impl Into<PiecePosition>) -> Vec<PieceMove>;
}

impl GetAvailableMoves for Board {
    fn get_available_moves(&self, pos: impl Into<PiecePosition>) -> Vec<PieceMove> {
        let pos = pos.into();
        ...
        match **cell {
            ...
            Some(piece) => match piece.ty {
                PieceTy::PAWN => {}
                PieceTy::ROOK => vec.extend(geodesic_calculator(pos, piece.color, .., ..0, self)),
                PieceTy::KNIGHT => {
                    vec.extend(
                        shape_geodesic_field::KNIGHT_FIELD[*pos]
                            .iter()
                            .filter_map(|&i| match self.at(i).value {
                                Some(at) => {
                ...
                PieceTy::BISHOP => vec.extend(geodesic_calculator(pos, piece.color, ..0, .., self)),
                PieceTy::QUEEN => vec.extend(geodesic_calculator(pos, piece.color, .., .., self)),
                PieceTy::KING => vec.extend(geodesic_calculator(pos, piece.color, ..1, ..1, self)),
                PieceTy::MAGE => vec.extend(geodesic_calculator(pos, piece.color, ..1, ..1, self)),
            },
        }
        vec
    }
